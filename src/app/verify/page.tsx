...
export default function VerifyPage() {
  ...
  const [step, setStep] = useState<'complete' | 'done'>('complete')
  ...
  const handleComplete = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setIsLoading(true)
    try {
      const response = await fetch(apiUrl('/api/users/complete-registration'), {
        method: 'POST',
      ...
      <Container>
        <div className="mx-auto max-w-xl rounded-2xl border border-slate-200 bg-white p-8 shadow-lg dark:border-gray-800 dark:bg-gray-900">
          ...
          <h1 className="mt-2 text-2xl font-semibold text-gray-900 dark:text-white">
            Complete your Wayfinder registration
          </h1>
          <p className="mt-2 text-sm text-slate-600 dark:text-gray-300">
            Verify your email token, then set a password to activate your account.
          </p>
